...
function makeApiCallWriteQApnl() {
	  var params = {
        ...
        // The A1 notation of the values to update.
        range: 'QApnl!A2:BE60',  // TODO: Update placeholder value.
      ...
      var valueRangeBody = {
		"range": "QApnl!A2:BE60",  //Set this to cell want to add 'x' to.
		"majorDimension": "ROWS",
		"values": [
...
function pushQApnl(order, offer_id){
	...
	xhttp2.open("GET", "https://psflc.api.hasoffers.com/Apiv3/json?NetworkToken=NETvgwPirxWahAF3mj5WHJs2HT5tLv&Target=Report&Method=getStats&fields[]=Stat.date&fields[]=Offer.name&fields[]=Stat.clicks&fields[]=Stat.conversions&fields[]=Stat.revenue&fields[]=Stat.payout&filters[Stat.date][conditional]=BETWEEN&filters[Stat.date][values][]="+year+"-"+month+"-01&filters[Stat.date][values][]="+year+"-"+month+"-"+days+"&filters[Offer.name][conditional]=LIKE&filters[Offer.name][values][]=%Push+Flow%&limit=1000", true);
	setTimeout(xhttp2.send(), 100);
}
